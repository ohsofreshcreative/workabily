...
class DropdownWalker extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
	{
		...
		if ($depth === 0 && $has_children) {
			...
			$output .= '<a href="' . esc_attr($item->url) . '" class="inline-flex items-center gap-x-1 text-sm font-medium !hover:text-white">';
		...
		// Case 2: Pozostałe elementy (zwykłe linki na górze lub linki wewnątrz dropdownu)
			...
			if ($depth > 0) {
				...
				$link_classes = 'text-lg font-medium text-white hover:text-secondary transition-colors duration-400';
			}
